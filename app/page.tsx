  ...
  return (<div className="min-h-screen bg-[#07090B] text-white">
    ...
    <section id="features" className="mx-auto max-w-7xl px-6 md:px-10 py-16">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { icon: Brain, title: "AI Discovery", desc: "Flags promising new tokens early." },
          { icon: ChartLine, title: "Momentum Signals", desc: "Volume, whales, liquidity changes." },
          { icon: Bell, title: "Smart Alerts", desc: "Email/push, wallet watch, thresholds." },
        ].map(({icon:Icon,title,desc},i)=>(
          <GradientCard key={title}><CardContent className="p-6">
            {/* @ts-ignore */}<Icon className="h-6 w-6 text-cyan-400"/><h3 className="mt-3 font-semibold text-white">{title}</h3><p className="mt-2 text-sm text-gray-400">{desc}</p>
