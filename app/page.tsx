...
const EMBEDDED_LOGO_DATA_URI = "data:image/svg+xml;utf8," + encodeURIComponent(`<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 420 120'><defs><linearGradient id='g' x1='0' x2='1' y1='0' y2='1'><stop offset='0%' stop-color='#22d3ee'/><stop offset='100%' stop-color='#34d399'/></linearGradient></defs><text x='50%' y='55%' text-anchor='middle' dominant-baseline='middle' font-family='Inter, system-ui, -apple-system, Segoe UI, Roboto, Ubuntu, Cantarell, Noto Sans, sans-serif' font-size='52' font-weight='900' fill='url(#g)'>SurgeX</text></svg>`);

const DEFAULT_LOGO = "/surgex-logo.png";
const ENV_LOGO = (typeof process !== "undefined" && (process as any).env?.NEXT_PUBLIC_LOGO_PATH) || undefined;
const LOGO_PATH = ENV_LOGO && !String(ENV_LOGO).startsWith("file:") ? String(ENV_LOGO) : DEFAULT_LOGO;

const FALLBACK_LOGO_SVG = EMBEDDED_LOGO_DATA_URI;

// Demo media config
const DEMO_EMBED_URL = ""; // set to a YouTube/Loom embed to show iframe
const DEMO_IMAGE = "/demo-screenshot.png"; // fallback image
...
function LogoImg({ width = 160, height = 54, className = "" }: {width?:number;height?:number;className?:string}){
  ...
  return (
    <img
      ...
      decoding="async"
      onError={() => {
        if (src !== EMBEDDED_LOGO_DATA_URI) { setSrc(EMBEDDED_LOGO_DATA_URI); return; }
        if (src !== FALLBACK_LOGO_SVG) setSrc(FALLBACK_LOGO_SVG);
      }}
      ...
      {!loaded && (
        <div className="absolute inset-0 grid place-items-center rounded-xl border border-white/10 bg-white/5 text-gray-300 text-sm">
          <div className="text-center">
            <p className="mb-2">Demo snapshot coming soon.</p>
            <p>Place an image at <code>/public/demo-screenshot.png</code>.</p>
          </div>
        </div>
      )}
    </div>
...
export default function SurgeXLanding(){
  ...
  async function onSubmit(e:any){
    ...
    try{ setStatus("submitting"); await new Promise(r=>setTimeout(r,600)); setStatus("success"); setMessage("You're on the list! We'll ping you at launch."); setEmail(""); } 
    catch{ setStatus("error"); setMessage("Something went wrong. Please try again."); }
  }

  useEffect(()=>{
    if(process.env.NODE_ENV==="production") return;
    console.assert(!HEADLINE.includes("#"), "Headline must not include '#'");
  },[]);
  ...
  return (
    <div className="min-h-screen bg-[#07090B] text-white selection:bg-cyan-500/20 selection:text-cyan-200">
      ...
      <section id="features" className="mx-auto max-w-7xl px-6 md:px-10 py-24">
        ...
        <motion.p {...fadeUp(1)} className="text-gray-400 max-w-3xl mb-10">Purpose-built for memecoins: speed, signals, and social context. Our AI helps filter noise and surface what actually moves.</motion.p>
      ...
      <section id="token" className="mx-auto max-w-7xl px-6 md:px-10 py-24">
        ...
        <motion.p {...fadeUp(1)} className="text-gray-400 max-w-3xl mb-10">SURGEX (<span className="text-white/90 font-semibold">$SRGX</span>) is the utility token powering access to premium AI signals, staking tiers, trading fee discounts, and governance on SurgeX. Contract: <span className="text-gray-300">TBA (after audit)</span>.</motion.p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <motion.div {...fadeUp(2)}><GradientCard><CardContent className="p-6"><div className="text-sm text-cyan-400">Total Supply</div><div className="mt-1 text-2xl font-bold text-white">1,000,000,000</div><p className="text-xs text-gray-400 mt-2">Fixed supply • 9 decimals</p></CardContent></GradientCard></motion.div>
          <motion.div {...fadeUp(3)}><GradientCard><CardContent className="p-6"><div className="text-sm text-cyan-400">Ticker / Chain</div><div className="mt-1 text-2xl font-bold text-white">SRGX / Solana (SPL)</div><p className="text-xs text-gray-400 mt-2">Solana SPL token — low fees & speed. Contract address will be published after audit.</p></CardContent></GradientCard></motion.div>
          <motion.div {...fadeUp(4)}><GradientCard><CardContent className="p-6"><div className="text-sm text-cyan-400">Launch</div><div className="mt-1 text-2xl font-bold text-white">TBD</div><p className="text-xs text-gray-400 mt-2">Fair, transparent, and bot-mitigated. Details in the litepaper.</p></CardContent></GradientCard></motion.div>
        </div>
        <motion.div {...fadeUp(5)} className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-6">
          <GradientCard><CardContent className="p-6"><h3 className="font-semibold text-white">Utility</h3><ul className="mt-3 space-y-2 text-sm text-gray-300">{["Stake to unlock Pro/Elite signal tiers","Fee discounts on trade routing and alerts","Access to priority listings & beta features","Governance on feature roadmap & emissions"].map((t,i)=>(<li key={i} className="flex items-start gap-2"><Check className="mt-0.5 h-4 w-4 text-emerald-400"/><span>{t}</span></li>))}</ul></CardContent></GradientCard>
          <GradientCard><CardContent className="p-6"><h3 className="font-semibold text-white">Tokenomics (draft)</h3><div className="mt-4 space-y-3">{[{label:"Liquidity",p:20},{label:"Community & Incentives",p:30},{label:"Team (24m vest)",p:15},{label:"Treasury",p:20},{label:"Partners & Advisors (18m vest)",p:10},{label:"Reserve",p:5}].map(a=>(<div key={a.label}><div className="flex justify-between text-xs text-gray-400"><span>{a.label}</span><span>{a.p}%</span></div><div className="mt-1 h-2 rounded-full bg-white/5 overflow-hidden"><div className="h-full bg-gradient-to-r from-cyan-400 to-emerald-400" style={{width:`${a.p}%`}}/></div></div>))}</div><p className="mt-3 text-xs text-gray-500">Draft figures • subject to change after audits and market conditions.</p></CardContent></GradientCard>
        </motion.div>
        <div className="mt-10 flex flex-wrap items-center gap-3"><Button className="rounded-xl bg-gradient-to-r from-cyan-400 to-emerald-400 text-black font-bold">Read Litepaper</Button><Button variant="outline" className="rounded-xl border-white/10 text-white hover:bg-white/5">View Tokenlist</Button></div>
      </section>
        ...
        <div className="relative pl-6">
          ...
          {[
            ...
            <motion.div key={m.q} {...fadeUp(i)} className="relative mb-8">
              ...
              <div className="ml-4"><div className="text-sm text-cyan-400">{m.q}</div><div className="text-white font-semibold">{m.t}</div><p className="text-gray-400 text-sm mt-1">{m.d}</p></div>
            </motion.div>
          ))}
        </div>
      </section>
