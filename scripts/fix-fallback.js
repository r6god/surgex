// scripts/fix-fallback.js
const fs = require("fs");
const path = require("path");

const file = path.join(process.cwd(), "app", "page.tsx");
if (!fs.existsSync(file)) process.exit(0);

let text = fs.readFileSync(file, "utf8");
const base64 = "data:image/svg+xml;base64,";

const pattern = /const\s+FALLBACK_LOGO_SVG\s*=\s*[^;]+;/m;
if (!pattern.test(text)) process.exit(0);

text = text.replace(
  pattern,
  `const FALLBACK_LOGO_SVG = "${base64}";`
);

// Remove any accidentally concatenated encodeURIComponent(...) SVG or stray PNG base64 lines
text = text
  .replace(/utf8",\s*\+\s*encodeURIComponent\(`[\s\S]*?\`\);?/g, "")
  .replace(/^\s*base64,[A-Za-z0-9+/=]+.*$/gm, "");

fs.writeFileSync(file, text, "utf8");
console.log("✔ fix-fallback.js: normalized FALLBACK_LOGO_SVG");
